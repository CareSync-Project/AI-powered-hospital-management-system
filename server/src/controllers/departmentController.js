...
export const departmentController = {
  ...
  async update(request, response) { const data = await departmentService.update(request.params.id, request.body); await auditService.record({ userId: request.auth.userId, hospitalId: data.hospitalId, action: request.body.active === false ? 'DEPARTMENT_DEACTIVATED' : 'DEPARTMENT_UPDATED', resourceType: 'Department', resourceId: data.id, request }); response.json({ success: true, data }); },
};
